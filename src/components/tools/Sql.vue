<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>高风险SQL下降率监控</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    >
      ...
      >
        ...
        <el-form-item label="子系统名称" prop="interval">
    ...
    <el-table :data="sqlList" stripe border>
      <el-table-column label="序号" type="index"></el-table-column>
      <el-table-column label="项目名称" prop="diffName"></el-table-column>
      <el-table-column label="子系统" prop="interval"></el-table-column>
      <el-table-column label="邮箱" prop="resp_overtime"></el-table-column>
...
export default {
  ...
  methods: {
    async getDiffList() {
      ...
      this.sqlList = res.data.diffs;
      this.total = res.data.total;
      console.log(this.sqlList);
    },

    async diffStateChange(sqlInfo) {
      const { data: res } = await this.$http.put(
        `sql/${sqlInfo.id}/state/${sqlInfo.mg_state}`
      );
      if (res.meta.status !== 200) {
        sqlInfo.mg_state = !sqlInfo.mg_state;
    ...
    addProject() {
      this.$refs.addFormRef.validate(async (validate) => {
        ...
        const { data: res } = await this.$http.post("sql", this.addForm);
    ...
    async showEditProject(id) {
      const { data: res } = await this.$http.get(`sql/${id}`);
      if (res.meta.status !== 200) {
    ...
    deleteProjectById(id) {
      this.$confirm("确定删除该项目, 是否继续?", "提示", {
        ...
        .then(async () => {
          const { data: res } = await this.$http.delete(`sql/${id}`);
          if (res.meta.status !== 200) {
